...
C
      ...
      CHARACTER(*), PARAMETER :: outplace = "../OUT/"
...
C
      ...
      PRINT '('' ****''/)'
C      PRINT '('' INPUT AND OUTPUT FILES ARE IN "TASK-2000" FORMAT''/)'
      PRINT '('' INPUT CONTROL FILE NAME (eq. Cochin_2012_tira.ctl )'')'
      READ(*,*)input
      PRINT '('' OUTPUT FILE NAME (eq. Cochin_2012_tira.pri )'')'
      READ(*,*)output

      write(*,*) inputplace//input
      OPEN(5,FILE=inputplace//input,STATUS='OLD',ERR=974)
      OPEN(7,FILE=outplace//output,STATUS='UNKNOWN',ERR=975)
C
      READ(IR,1056)
      READ(IR,1020) FILEIN
C      WRITE(6,*)
